...
void getRoute(vector<Node> v, int sourceIndex, int dest, int &best, int time1){
    time_t current = time(NULL);
   
    if(difftime(current, start) > 130){/*cout<<"return timer"<<endl;*/;return;}
    if(time1 >= best){ return;}
    if(v[sourceIndex].dest == dest) if(time1 < best) { cout<<" return time = "<<time1<<endl; best = time1;  return;}
     for(int i = 0; i<v[sourceIndex].adj.size();i++){ // per ogni adiacente
         int c = v[sourceIndex].adj[i]; // indice corrispondente su v
         if(time1 < v[c].startLav){
             if(time1 < v[c].timeFree) { // faccio una chiamata con quel time perchè aspetto per passare e poi vado nel adiacente strada
                  getRoute(v,c,dest,best,v[c].timeFree+1);
             }
             else{//non devo aspettare posso già passare
                 getRoute(v,c,dest,best,time1+1);
             }
        }
     }
}
