...
int main(){
    ...
    while(i<dim){
      ...
      /* if(i == 0) ret = findWithIntPsw(vec[i],m[i],1,4);
        else if (i == 1)ret = findWithIntPsw(vec[i],m[i],2,3);
        else ret = findWithIntPsw(vec[i],m[i],2,3);*/
...
string findWithIntPsw(string a, int m, int maxInterGroup, int maxInterNum){
    ...
    for(int i=0; i< a.length()-m+1;i++){
        ...
        if(!eq){ // se non ci sono caratteri uguali nella parola presa in considerazione
            ...
            for(int j = 0; j<a.length()-m;j++){ //cerco una posizione di inizio
                if(w.s[0] == a[j]){
                    ...
                    while(c < m && c+ind<a.length() && (w.s[c] == a[c+ind] || canCountInter < maxInterGroup)){
                        if(w.s[c] == a[c+ind])c++;
                        else{ //sono diversi
                            canCountInter+=1;
                            int inter = 0;
                            while(inter < maxInterNum && w.s[c] != a[c+ind] && c+ind < a.length()) {
                                inter++;
                                ind++;
                            }
                        }
            ...
            if(w.count ==  maxRepeated.count)cout<<"trovati count uguali "<<w.s<<endl;
            if(w.count > maxRepeated.count) maxRepeated = w;
        }
    }
    cout<<"count= "<<maxRepeated.count<<endl;
