...
public:
    ...
    int height(Nodo<K>* n){
        if(n == NIL) return 0;
        return max(height(n->getChildSx()),height(n->getChildDx())) + 1;
    }
    ...
    void insert_fixup(Nodo<K>* z){
        if(z == root){
           // cout<<"nonno è radice"<<endl;
            z->setBlack();
            return;
        }
        if(z->getParent()->isRed() ==  false) return;
        //è figlio di un nodo rosso
...
int main(){
	...
	for(int i=0; i<100;i++){
        ...
        if(type == "int"){
            ...
            out<< A.call_height() <<endl;
            //A.call_preorder(); cout<<A.pre<<endl;
